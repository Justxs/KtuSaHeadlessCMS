    ...
    : Endpoint<GetEventsBySaUnitRequest, List<EventPreviewResponse>>
{
    public override void Configure()
    {
        ...
        Description(b => b
            ...
            .WithDescription("Returns event previews organised by the specified SA unit, ordered by start date descending. Language: 'lt' or 'en'. Pass fetchPassed=true to include past events. Allowed saUnit values: CSA, InfoSA, Vivat_Chemija, InDi, STATIUS, FUMSA, ESA, SHM, VFSA, BRK.")
    ...
    public override async Task HandleAsync(GetEventsBySaUnitRequest req, CancellationToken ct)
    {
        ...
        var eventDtos = events
            .Where(eventItem => eventItem.As<EventPart>().OrganisersField.ContentItemIds.Contains(saUnit.ContentItemId))
